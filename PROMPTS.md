...
## Prompt #15: UI State Accuracy, Layout Consistency, and Session Restoration

> I need a careful fix for several persistent frontend issues. The dashboard continues to show tasks as in progress after backend work has already completed. For example, "resolving links" and "classifying sentences" remain in a loading state even when those steps are done. Update the UI so each status reflects the true backend process lifecycle, and ensure completed steps are marked finished without lingering spinners. Also adjust the source domains panel so its height matches the agent orchestration panel for consistent visual balance. There is also a navigation bug in session history. After selecting "Check another" and then clicking a previous query, the app sometimes renders a blank white screen instead of returning to the populated dashboard. Fix this flow so selecting any prior query reliably restores the dashboard view with that query's data loaded. Plan this carefully, implement the fixes thoroughly, and verify the behavior end to end before considering it complete. Finally, resolve a scroll lock regression where after sufficient vertical scrolling the main viewport stops accepting wheel or trackpad input, which traps the user partway down the page with no way to scroll back up or use the layout normally until a full refresh. Trace nested overflow containers, fixed or sticky stacking contexts, global overflow hidden on root elements during modals or transitions, and wheel or touch handlers that may be capturing or canceling default scroll behavior, and restore predictable document level scrolling across the full content height.

---

## Prompt #14: Completion State Accuracy, Session Navigation, and Query Labels

> The orchestration layer and verdict pipeline behave correctly end to end, including delivery of all fourteen claim verdicts. The remaining work is client state synchronization and navigation. First, reconcile the dashboard completion signal with per agent and per task loading indicators so that when the run is finished and the summary reflects completion, no subordinate UI still reports indeterminate or in flight status. Tie those indicators to the same authoritative job or run state the verdict list uses, and clear or finalize spinners and progress affordances when the backend marks the job complete. Second, fix session history navigation as a priority regression. After the user chooses Check another page and then selects a prior query from the left session list, the app must route to that query dashboard with populated state. Today it renders an empty viewport with no in app recovery path, which forces a full page reload. Trace selection handling, route parameters or store hydration, and any lazy or suspense boundaries so the selected job id consistently loads results and the layout never sticks on a blank shell. Third, replace opaque sidebar titles such as FAQ5 with human meaningful labels derived from fetched metadata, for example registered domain, site display name, or article title when available, with a short deterministic fallback only when metadata is missing. Acceptance criteria are visually consistent terminal states on completion, reliable reopening of any session query without a blank route, and labels that identify each run at a glance.
